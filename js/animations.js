...
export function renderCellAnimation(cell, [x1, y1], [x2, y2]) {
    ...
    return cellCopy.animate(
        translateAnimKeyframes(x1, y1, x2, y2),
        translateAnimOptions
    ).finished;
}
...
export function animateSliding(board) {
    ...
    for (let index = 0; index < board.boardCellsData.length; index++) {
        ...
        if (board.boardCellsData[index].isMerged) {
            const mergedIndex = board.boardCellsData.findIndex(
                (cell) => cell.id === board.boardCellsData[index].mergedTo
            );
...
export function animateMerging(board) {
    ...
    for (let index = 0; index < board.boardCellsData.length; index++) {
        if (board.boardCellsData[index].isMerged) {
            const indexMerged = board.boardCellsData.findIndex(
                (cell) => cell.id === board.boardCellsData[index].mergedTo
            );
            animations.push(
                board.boardCellsHTML[indexMerged].animate(
                    scaleAnimKeyframes,
                    scaleAnimOptions
                )
            );
